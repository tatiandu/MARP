...
protected:
	...
	struct TreeNode {
		...
		Link iz, dr;
		int altura;
		int tam_i; // posicion que ocuparia si se ordenasen los elementos
...
protected:
	const T& kesimo(Link a, int k) const {
		T n = a->tam_i;

		if (n == k)
			return a->elem;

		if (n > k) //: buscamos en la izquierda
			return kesimo(a->iz, k);

		//const auto dr = a->dr;
		//if (dr == nullptr) //n < k: buscamos en la derecha
		//	throw std::runtime_error("error: invalid index");

		//llamadas recursivas con k = k - n
	...
	void rotaDer(Link& r2) {
		...
		r2->tam_i = r2->tam_i - r1->tam_i; //TAMI
		r2 = r1;
	}
	...
	void reequilibraDer(Link& a) {
		if (altura(a->iz) - altura(a->dr) > 1) {
			if (altura(a->iz->dr) > altura(a->iz->iz))
				...
				rotaDer(a);
				if (a->iz != nullptr) //Si tiene hijo izq
